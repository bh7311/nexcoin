...
#if defined(NDEBUG)
# error "Nexcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x26a727048ba297112c6f7980ffd0e9a981a42f457bd778368585b09f635d488b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nexcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nexcoin Signed Message:\n";
...
{
    // Nexcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Nexcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Nexcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Nexcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nexcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nexcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x26a727048ba297112c6f7980ffd0e9a981a42f457bd778368585b09f635d488b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Nexcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x32ef3c327e8342c758abb04524ff36c8f7be128dd7e04d0d24f1b8816ab3737a"));
	...
	if(false && block.GetHash() != hashGenesisBlock)
	{
		...
		loop
		{
			...
			if(block.nNonce == 0)
			{
				printf("NONCE WRPPED, incrementing time\n");
				++block.nTime;
			}
		}
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Nexcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NexcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NexcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NexcoinMiner(CWallet *pwallet)
{
    printf("NexcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nexcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NexcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NexcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NexcoinMiner, pwallet));
}
